...
- `test/plot.py`: seismic visualization utility.
...
The repository provides a complete seismic denoising example in `example_data/`:

- `example_noisy.npy`: noisy seismic section used as input.
- `example_denoised.npy`: denoised result generated by the pretrained model.
- `noisy.png`: visualization of the noisy seismic section.
- `denoised.png`: visualization of the denoised seismic section.

The `.npy` files are stored as 2D `float32` NumPy arrays with shape `(H, W)`, where:
- `H` represents the temporal dimension (time samples).
- `W` represents the spatial dimension (trace number).

A visualization utility is provided in:

```text
test/plot.py
```

The script renders seismic sections using a professional seismic colormap (`seismic`) and automatically scales amplitudes using percentile clipping.
...
The plotting function supports:
- Custom trace/time axis ranges.
- High-resolution image export.
- Automatic amplitude normalization.
- Seismic-specific visualization styles.

Generated figures are suitable for:
- qualitative denoising comparison,
- paper visualization,
...
|---|---|
| ![](example_data/noisy.png) | ![](example_data/denoised.png) |
